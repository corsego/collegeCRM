...
class UsersController < ApplicationController
  ...
  def index
    @users = if params[:email]
               User.where('email ILIKE ?', "%#{params[:email]}%").order(created_at: :desc) # case-insensitive
             else
  ...
  def require_admin_or_inviter
    @user = User.find(params[:id])
    redirect_to (request.referer || root_path), alert: 'You are not authorized to perform this action' unless current_user.admin? || @user.invited_by == current_user
  end

  def require_admin_or_owner
    @user = User.find(params[:id])
    redirect_to (request.referer || root_path), alert: 'You are not authorized to perform this action' unless current_user.admin? || current_user == @user
  end
end
